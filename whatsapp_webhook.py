...
def format_for_tts(raw_text: str) -> str:
    ...
    """
    system_prompt = """
    אתה מבצע התאמה קלה לטקסט קיים כדי שיהיה נוח להקראה ב-TTS של ElevenLabs.
    כללים מחייבים לפלט:
    - שמור את גוף הפנייה, הכינויים, הטון והסגנון בדיוק כפי שהתקבלו. אל תחליף "אתה/את" ל"אתם" או להפך. אל תשנה רשמי/לא-רשמי.
    - אל תשנה משמעות, אל תוסיף מידע, אל תחסיר מידע, אל תקצר ואל תאריך.
    - אל תסכם ואל תנסח מחדש. שמור על סדר המשפטים המקורי ככל האפשר.
    - בצע רק תיקוני פיסוק קלים (נקודות, פסיקים) ויישור רווחים/שבירות שורה לפי הצורך להקראה ברורה.
    - אל תשתמש בתגיות, בסוגריים מיוחדים או בקוד. טקסט רגיל בלבד.
    """
    try:
